package com.may.taoba.javascript;
...
import com.may.taoba.ActivityContainer;
import com.may.taoba.HeaderWebActivity;
import com.may.taoba.bean.CitiesList;
import com.may.taoba.bean.City;
import com.may.taoba.bean.Preferences;
import com.may.taoba.module.jsinterface.ScriptInterface;
import com.may.taoba.tools.StringTool;

public class Bridge extends ScriptInterface{
	...
	public String getCity(){
		...
		if (StringTool.isEmpty(s).booleanValue()){
			s1 = "{name: '北京', id: 1, pinyin: 'beijing'}";
		} else{
			//根据city的id来获取相应的city
			City city = CitiesList.getInstance().getCityById(s);
			if (city == null){
				s1 = "{name: '北京', id: 1, pinyin: 'beijing'}";
			} else{
	...
	public void getCityList(){
		final String ret = CitiesList.getInstance().getAllJson().replace("\\n", "").trim();
	...
	public void openWebActivity(String s){
		...
		try {
			...
			//转换成JSONObject对象
			jsonobject = StringTool.parseJSON(s);
		...
		implements Runnable
	{
		...
		public void run()
		{
			...
			String s = ret;
			as[0] = s;
			headerwebactivity.CallJSFunction("_on_citylist_finished", as);
		}
